# Terraform script to create the common resources for PDS Nucleus

resource "aws_security_group" "nucleus_security_group" {
  name        = "nucleus_security_group"
  description = "nucleus_security_group"
  vpc_id      = var.vpc_id

  ingress {
    description = "All traffic"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
    cidr_blocks = var.nucleus_security_group_ingress_cidr
  }

  egress {
    from_port        = 0
    to_port          = 0
    protocol         = "-1"
    cidr_blocks      = ["0.0.0.0/0"]
    ipv6_cidr_blocks = ["::/0"]
  }
}


#resource "aws_s3_bucket" "nucleus_airflow_dags_bucket" {
#  bucket = var.mwaa_dag_s3_bucket_name
#
#  tags = {
#    Name        = "Nucleus Airflow DAGS bucket"
#    Environment = "Dev"
#  }
#
#  force_destroy = true
#}
#
#resource "aws_s3_bucket_versioning" "nucleus_dags_bucket_versioning" {
#  bucket = aws_s3_bucket.nucleus_airflow_dags_bucket.id
#  versioning_configuration {
#    status = "Enabled"
#  }
#}
#
#resource "aws_s3_bucket_server_side_encryption_configuration" "nucleus_dags_bucket_encryption" {
#  bucket = aws_s3_bucket.nucleus_airflow_dags_bucket.id
#
#  rule {
#    apply_server_side_encryption_by_default {
#      sse_algorithm = "AES256"
#    }
#  }
#}

data "aws_s3_bucket" "nucleus_airflow_dags_bucket" {
  bucket = var.mwaa_dag_s3_bucket_name
}

#resource "aws_s3_object" "dags" {
#  bucket        = data.aws_s3_bucket.nucleus_airflow_dags_bucket.id
#  acl           = "private"
#  key           = "dags/"
#  source        = "/dev/null"
#  force_destroy = true
#  depends_on    = [data.aws_s3_bucket.nucleus_airflow_dags_bucket]
#}
#
#resource "aws_s3_object" "requirements" {
#
#  bucket        = data.aws_s3_bucket.nucleus_airflow_dags_bucket.id
#  key           = "requirements.txt"
#  acl           = "private"
#  force_destroy = true
#  source        = "./terraform-modules/mwaa-env/requirements.txt"
#}

#resource "aws_s3_bucket_public_access_block" "nucleus_airflow_dags_bucket_public_access_block" {
#  bucket = aws_s3_bucket.nucleus_airflow_dags_bucket.id
#
#  block_public_acls       = true
#  block_public_policy     = true
#  ignore_public_acls      = true
#  restrict_public_buckets = true
#}
#
#resource "aws_s3_bucket_policy" "allow_access_from_another_account" {
#  bucket = aws_s3_bucket.nucleus_airflow_dags_bucket.id
#  policy = data.aws_iam_policy_document.allow_access_from_another_account.json
#}


#data "aws_iam_policy_document" "allow_access_from_another_account" {
#  statement {
#    principals {
#      type        = "AWS"
#      identifiers = [aws_iam_role.pds_nucleus_mwaa_execution_role.arn]
#    }
#
#    actions = [
#      "s3:*"
#    ]
#
#    effect = "Allow"
#
#    resources = [
#      aws_s3_bucket.nucleus_airflow_dags_bucket.arn,
#      "${aws_s3_bucket.nucleus_airflow_dags_bucket.arn}/*",
#    ]
#  }
#}

#output "pds_nucleus_mwaa_execution_role_arn" {
#  value = aws_iam_role.pds_nucleus_mwaa_execution_role.arn
#}

output "pds_nucleus_security_group_id" {
  value = aws_security_group.nucleus_security_group.id
}

output "pds_nucleus_airflow_dags_bucket_arn" {
  value = data.aws_s3_bucket.nucleus_airflow_dags_bucket.arn
}
