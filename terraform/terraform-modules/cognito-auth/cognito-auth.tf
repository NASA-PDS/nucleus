...
resource "null_resource" "archive_lambda_package" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      if [ ! -d "package" ] || [ -z "$(ls -A package)" ]; then
        echo "Package directory missing or empty — rebuilding..."
        bash build-lambda.sh
      fi

      zip -r pds_nucleus_alb_auth_layer.zip package >/dev/null
      echo "Lambda package zipped: ${path.module}/lambda/pds_nucleus_alb_auth_layer.zip"
    EOT
  }
...
data "archive_file" "pds_nucleus_auth_alb_function_zip_packages" {
  ...
  depends_on  = [null_resource.archive_lambda_package]
}
...
resource "null_resource" "create_cognito_groups_if_missing" {
  for_each = { for g in local.cognito_groups : g.name => g }

  provisioner "local-exec" {
    command = <<EOT
      set -e
      echo "Checking Cognito group: ${each.value.name}"
      exists=$(aws cognito-idp get-group --group-name ${each.value.name} --user-pool-id ${data.aws_cognito_user_pool.cognito_user_pool.id} 2>/dev/null || true)
      if [ -z "$exists" ]; then
        echo "Creating Cognito group: ${each.value.name}"
        aws cognito-idp create-group \
          --group-name ${each.value.name} \
          --user-pool-id ${data.aws_cognito_user_pool.cognito_user_pool.id} \
          --description "${each.value.description}" \
          --precedence ${each.value.precedence}
      else
        echo "Group already exists: ${each.value.name}, skipping."
      fi
    EOT
  }
...
output "pds_nucleus_airflow_ui_urls" {
  value = [
    "https://${aws_lb.pds_nucleus_auth_alb.dns_name}/aws_mwaa/aws-console-sso",
    "https://${var.nucleus_cloudfront_origin_hostname}/nucleus"
  ]
}
