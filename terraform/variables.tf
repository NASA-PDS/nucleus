...
variable "pds_data_source_names" {
  description = "List of data source identifiers, one entry per data source (e.g. 'backlog', 'realtime'). A single PDS node can have multiple data sources, each getting its own S3 config bucket/SQS queue/Lambda pair, while sharing the node's IAM role and staging bucket."
  type        = list(string)
}
