...
print('Loading pds-nucleus-product-completion-checker function')
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Data file received
            ...
            if not s3_url_of_data_file.lower().endswith("/"): # Not a directory
                copy_file_to_efs(bucket, key, efs_mount_path)
...
def handle_missing_files_for_product_label(s3_url_of_product_label, bucket, key):
    ...
    print('Get s3_response for key {} from bucket {}'.format(key, bucket))
    ...
    try:
        ...
        for x in missing_files_from_product_label:
            ...
            if items:
                print('The file is already received: ' + items['s3_url_of_data_file'])
            else:
...
# Returns an expected files response (with list of product labels) for a given s3_url_of_data_file
def get_expected_file_items(s3_url_of_data_file):
...
def update_existing_incomplete_products(s3_url_of_data_file):
    expected_files_items = get_expected_file_items(s3_url_of_data_file)
    ...
    if expected_files_items:
        ...
        print("expected_files_product_label_list = " + str(expected_files_product_label_list))
...
def update_expected_files(s3_url_of_data_file, s3_url_of_product_label):
    ...
    if expected_files_items:
        expected_files_product_label_set = expected_files_items['s3_urls_of_product_labels']
...
def copy_file_to_efs(s3_bucket, s3_key, efs_mount_path):
    ...
    try:
        ...
        print(f"File downloading to : {download_file_path}")
    ...
    try:
        ...
        if os.path.isfile(os.path.normpath(destination_file)):
            ...
            print(f"Deleted existing file in: {os.path.normpath(destination_file)}")
...
def notify_product(s3_url_of_product_label):
    ...
    print('SQS Message sent for the completed product')

# Uses to convert set to list for JSON serializing 
def set_default(obj):
